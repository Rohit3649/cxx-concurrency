...
int parallal_accumulate(iterator begin, iterator end)
{
    ...
    //comiler will decide if async task will run is current or new thread
	std::future<int> f1 = std::async(std::launch::deferred | std::launch::async, parallal_accumulate<iterator>, mid, end);
