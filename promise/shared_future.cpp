...
void print_result1(std::future<int>& fut)
{
    ...
    // this can cuase race condition. Both thread execute valid statement together but as soon as first thread  call get() - Future will 
    // become invalid. and when second thread will call get() - it will throw exception (or undefined behaviour)
    // Solution - user shared_future()
	if (fut.valid()) 
	{
